...
import { ListJsModel } from './listes.model';
...
function matches(country: ListJsModel, term: string, pipe: PipeTransform) {
  ...
  return (country.reference?.toLowerCase() ?? '').includes(searchTermb)
    || (country.adresseExpediteur?.toLowerCase() ?? '').includes(searchTermb)
    || (country.codeClient?.toLowerCase() ?? '').includes(searchTermb)
    || (country.Destinataire?.toLowerCase() ?? '').includes(searchTermb)
    || (country.adresseDestinataire?.toLowerCase() ?? '').includes(searchTermb)
    || (country.expediteur?.toLowerCase() ?? '').includes(searchTermb)
...
export class OrdersService {
 ...
 //   this.VehiculeService.getVehicules().subscribe(data => {
  ...
  getFactures() {
    ...
    this.FactureService.getLivraisonTermine().subscribe(data => {
