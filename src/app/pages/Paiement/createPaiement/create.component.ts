...
export class CreateComponent {
  ...
  saveFicheDeRoute() {
    ...
    const livraisonData = {
      listeLiv: this?.checkedList,

      client_id: this?.selectedClient?.id,
      dateFacture: this?.date,
      idsite: this.defaultSiege.id
    };

    // Validate if all necessary fields are filled
    if (!this.selectedClient || !this.date || !this.checkedList) {
      this.isFormInvalid = true;

      // Display alert when checkedList is empty or other fields are missing
      Swal.fire({
        title: 'Erreur',
        text: 'Veuillez sélectionner au moins une livraison , un client et une date.',
        icon: 'error',
    ...
    this.FactureService.createFacture(livraisonData).subscribe(response => {
      console.log('fiche de route saved successfully', response);

      // Refresh orders list after successful save
      this.getLivraisonTermine();

      // Show success notification
      let timerInterval: any;
      Swal.fire({
        title: 'Succès',
        text: 'Facture crée avec succès.',
        icon: 'success',
        timer: 2000,
  ...
  ajoutpaiement() {
    ...
    this.PaiementService.ajouterMultiplepaiment(paiementData).subscribe(response => {
      Swal.fire({
        title: 'succés',
  ...
  getLivraisonTermine() {
    ...
    this.FactureService.getLivraisonTermine().subscribe((data: any) => {
      ...
      this.searchResults = data.ListLivraison;
    });
  }
  changePage() {
    this.orderes = this.service.changePage(this.allorderes)
  ...
  performSearch(): void {
    this.searchResults = this.allorderes.filter((item: any) => {
      return (
        ...
        item?.codeClient?.toLowerCase().includes(this.searchTerm.toLowerCase() ?? '') ||
 ...
 */
  ...
  onCheckboxChange(e: any) {
    ...
    const index = this.orderes.findIndex((order: any) => order.id === idTrajet); // Updated key name
    if (index !== -1) {
  ...
  updateCheckedList() {
    ...
    this.checkedList = this.orderes
      ...
      .map((order: any) => ({ idliv: order.id, totalfrais: order.totalfrais, codeClient: order.codeClient, reference: order.reference })); // Updated key name
  }


  // Function to calculate the total of all totalfrais in the checkedList
  calculateTotalFrais() {
    // Check if checkedList is not null/undefined and has elements
    if (this.checkedList && this.checkedList.length > 0) {
      return this.checkedList.reduce((total: any, order: any) => total + order.totalfrais, 0);
    }
    // If checkedList is null/undefined or empty, return 0
    return 0;
  }
  ...
  */
  ...
  filterChauffeur() {
    ...
    if (this.selectedClient) {
      filteredOrders = filteredOrders.filter((order: any) =>
        order?.codeClient === this.selectedClient?.code
      );
    }
