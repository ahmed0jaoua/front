...
@Component({
  selector: 'app-orders',
  templateUrl: './listesEntrants.component.html',
  styleUrls: ['./listesEntrants.component.scss'],
 ...
 * Orders Component
 */
export class ListesEntrantsComponent {
  ...
  ngOnInit(): void {
    ...
    */
    this.breadCrumbItems = [
      ...
      { label: 'liste Entrant', active: true }
    ];
    ...
    this.BonlivraisonService.gerFicheEntrant(this.defaultSiege.id).subscribe((data: any) => {
  ...
  performSearch(): void {
    this.searchResults = this.allorderes.filter((item: any) => {
      return (
        item.fiche_id?.toString().includes(this.searchTerm.toLowerCase()) ||               
        item.nomvehicule?.toLowerCase().includes(this.searchTerm.toLowerCase()) || 
        item.nomchauffeur?.toLowerCase().includes(this.searchTerm.toLowerCase()) 
      );
    });
  ...
  onNavChange(changeEvent: NgbNavChangeEvent) {
    ...
    if (changeEvent.nextId === 1) {
      this.orderes = this.allorderes.filter((fiche: any) => fiche.etat == '0');
    }
 ...
 */
  ...
  */
  ...
  //       this.BonlivraisonService.deleteFicheDEroute(this.deleteId);
