...
export class ListesComponent {
  ...
  ngOnInit(): void {
    ...
    */
    ...
    this.PaiementService.getAllPaiements().subscribe((data: any) => {
      this.orderes = data.ListPaiement;
      this.allorderes = cloneDeep(data.ListPaiement);
      this.orderes = this.service.changePage(this.allorderes)
    });
  }
  ...
  performSearch(): void {
    this.searchResults = this.allorderes.filter((item: any) => {
      return (
       // Convert fiche_id to string for comparison
        item.reference?.toLowerCase().includes(this.searchTerm.toLowerCase() ??'' ) || // Convert fiche_id to string for comparison
        item.codeClient?.toLowerCase().includes(this.searchTerm.toLowerCase() ?? '') || // Convert fiche_id to string for comparison
        item.nomSiege?.toLowerCase().includes(this.searchTerm.toLowerCase() ?? '') || // Convert fiche_id to string for comparison
        item.datePaiment?.toLowerCase().includes(this.searchTerm.toLowerCase() ?? '') 
      );
    });
