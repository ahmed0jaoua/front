...
import { ListJsModel } from './siege.model';
...
function matches(country: ListJsModel, term: string, pipe: PipeTransform) {
  ...
  return (country.nom?.toLowerCase() ?? '').includes(searchTermb)
    || (country.code?.toLowerCase() ?? '').includes(searchTermb)
...
export class OrdersService {
  ...
  constructor(private pipe: DecimalPipe, public SiegeService: SiegeService ) {
  ...
  getSiege(){this.SiegeService.getSieges().subscribe(data => {
