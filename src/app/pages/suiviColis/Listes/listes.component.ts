...
export class ListesComponent {
  ...
  performSearch(): void {
    this.searchResults = this.allorderes.filter((item: any) => {
      return (
        item.fiche_id?.toString().includes(this.searchTerm.toLowerCase()) || // Convert fiche_id to string for comparison
        item.nomvehicule?.toLowerCase().includes(this.searchTerm.toLowerCase()) || // Convert fiche_id to string for comparison
