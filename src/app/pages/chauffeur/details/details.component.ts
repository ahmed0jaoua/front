...
export class DetailsComponent implements OnInit {
  ...
  originalbons: any[] = [];
  pageBon: any[] = [];
  listeLivraisonParChauffeur: any; 
  static: any;
  chauffeur: any ; 
  constructor(private formBuilder: UntypedFormBuilder, private BonlivraisonService: BonlivraisonService,
    private ClientService: ClientService, private route: ActivatedRoute, private modalService: NgbModal,
  ...
  ngOnInit(): void {
    ...
    this.BonlivraisonService.getStatistiques().subscribe((data: any) => {
      ...
      this.ChauffeurService.getChauffeur(id).subscribe((data: any) => {
      this.chauffeur = data;
      });
      this.ChauffeurService.getLivraisonParChauffeur(id).subscribe((data: any) => {
      this.listeLivraisonParChauffeur = data.ListLivraison;
    });
  ...
  getStatusTextc(statut: number | string): string {
    ...
    switch(statusNumber) {
      ...
      case 0: return 'Créé';  // ou tout autre texte approprié
  ...
  getBadgeClasses(statut: string): string {
    switch (statut) {
      ...
      default: return 'badge bg-secondary'; // Valeur par défaut
    }
  }
